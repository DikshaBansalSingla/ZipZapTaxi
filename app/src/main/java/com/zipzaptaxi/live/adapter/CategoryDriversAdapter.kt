...
class CategoryDriversAdapter (private val mContext: Activity, private val hint:String, private val listItems: ArrayList<DriverList>):
    BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
       var itemTitle=""
        if(position==0){
             itemTitle = listItems[position].name
        }else{
             itemTitle= listItems[position].name+ " ("+listItems[position].phone +")"
        }
